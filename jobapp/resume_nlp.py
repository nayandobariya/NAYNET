...
class ResumeAnalysis():
    ...
    def scoreresume(self,resumetext,resultpath):
        ...
        # Create pie chart visualization only if there's valid data
        if summary['score'].sum() > 0:
            ...
            plt.close(pie)  # Close the figure to free memory
        else:
            # Create a placeholder image or skip saving
            plt.figure(figsize=(10, 10))
            plt.text(0.5, 0.5, 'No data available for analysis', ha='center', va='center', fontsize=14)
            plt.title('Resume Analysis - No Data')
            plt.axis('off')
...
def rankall(file_paths, applicant_names):
    ...
    for i in range(0,len(file_paths)):
        print(applicant_names[i])
        text= ResumeAnalysisob.convertresumefileToText(file_paths[i])
        ...
        print(catresults)
        cvsresults.loc[i]=[applicant_names[i]]+catresults
